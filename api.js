...
app.get('/messages/contacts', verifierToken, (req, res) => {
    const utilisateurs = lireDB(fileUsers);
    const contacts = utilisateurs.filter(u => u._id !== req.userId).map(u => ({ _id: u._id, pseudo: u.pseudo, avatarUrl: u.avatarUrl }));
    res.json(contacts);
});
...
app.post('/posts', verifierToken, upload.single('image'), (req, res) => {
    ...
    // Détection dynamique du type de fichier injecté
    let mediaType = null;
    if (req.file) {
